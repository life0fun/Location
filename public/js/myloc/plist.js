...
function load_loc_db(reader) {
    return function(e) {
        ...
        console.log('db is '+data.length+' bytes');
    ...
    return function(e) {
        ...
        while(data.more()) {
            ...
            if(target == data.get_int(4)+6) {
                console.log('location db is in '+fileID);
                file = files[fileID];
                break;
            }
            data.skip(2);
        ...
        console.log('extracting location info; please wait...')
    }
}
...
function load_mbdb(reader,files) {
    return function(evt) {
       ...
       console.log('manifest is '+ data.length+' bytes');
